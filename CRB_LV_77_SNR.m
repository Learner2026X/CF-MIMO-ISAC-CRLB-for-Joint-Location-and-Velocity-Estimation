...
fs_integral = 1e6;% sampling rate for inregrating the discrete signals 
...
radius = 5e3; % radius of the circular distribution of access points
% azimuth_Tr = [85 65 45 25 5]; 
% azimuth_Re = [-85 -65 -45 -25 -5];
azimuth_Tr = [105 85 65 45 25 5 -15]+45; 
azimuth_Re = [-105 -85 -65 -45 -25 -5 15]-45;
p_tar = [0,0;0,0].'; % assumed location of the target of interest
% v_tar = [4,5].';
v_tar = [-15,0;4,5].'; % assumed velocity of the target of interest
p_tx = radius*[cosd(azimuth_Tr(1:numTr));sind(azimuth_Tr(1:numTr))]; % locations of transmitters
p_rx = radius*[cosd(azimuth_Re(1:numRe));sind(azimuth_Re(1:numRe))]; % locations of receivers
%% figure of sensing system
close all
figure
...
%% intermediate parameter for CRLB
...
%             cons_D(n,k) = 8*pi^2*transmission_energy*RCS_var(n,k)*Eff_time/noise_power(i_SNR)*fs; 
            ...
            xi(2,temp) = -1/lambda*((p_tar(2,i_tar)-p_tx(2,n))/...
               ...
               norm(p_tar(:,i_tar)-p_rx(:,k)));
            % the following three statements are to compute P,V,Y which are not used in this
            % program
...
CRB_loc_accurate(i_SNR,:) = [CRB_mat(1,1),CRB_mat(2,2)]; % accurate CRLB
CRB_vel_accurate(i_SNR,:) = [CRB_mat(3,3),CRB_mat(4,4)];
CRB_loc(i_SNR,:) = [CRB_mat_appr(1,1),CRB_mat_appr(2,2)];% approximate CRLB
...
%%
% save('data\CRB_loc77_T3.mat','CRB_loc')
% save('data\CRB_vel77_T3.mat','CRB_vel')
% save('data\CRB_loc77_accurate_T3.mat','CRB_loc_accurate')
% save('data\CRB_vel77_accurate_T3.mat','CRB_vel_accurate')