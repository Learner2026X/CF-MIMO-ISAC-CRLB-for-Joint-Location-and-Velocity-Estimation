...
% RCS(3:4,3:4) = 0.1*(ones(2,2)+1j*ones(2,2)); 
RCS_var = abs(RCS).^2;
CRB_loc = zeros(len_fs,2);
CRB_vel = zeros(len_fs,2);
CRB_loc_accurate = zeros(len_fs,2);
CRB_vel_accurate = zeros(len_fs,2);
for i_fs = 1:len_fs
    cons_f = 2*transmission_energy/noise_power*fs(i_fs);
%%
Num_Tar = 1;
for i_q = 1:  Num_Tar
...
%             cons_A(n,k) = 8*pi^2*transmission_energy*RCS_var(n,k)*Eff_bandw/noise_power(i_fs)*fs; % OCDM
%             cons_B(n,k) = 4*pi*transmission_energy*RCS_var(n,k)*TB/noise_power(i_fs)*fs; 
% %             cons_B = 0;
%             cons_D(n,k) = 8*pi^2*transmission_energy*RCS_var(n,k)*Eff_time/noise_power(i_fs)*fs; 
...
CRB_loc_accurate(i_fs,:) = [CRB_mat(1,1),CRB_mat(2,2)];
CRB_vel_accurate(i_fs,:) = [CRB_mat(3,3),CRB_mat(4,4)];
CRB_loc(i_fs,:) = [CRB_mat_appr(1,1),CRB_mat_appr(2,2)];
CRB_vel(i_fs,:) = [CRB_mat_appr(3,3),CRB_mat_appr(4,4)];
% rank(FIM(1:4,5:end))
% AA = FIM(1:4,1:4);
...
%%
% save('data\CRB_loc77_DF.mat','CRB_loc')
% save('data\CRB_vel77_DF.mat','CRB_vel')
save('data\CRB_loc77_accurate_fs.mat','CRB_loc_accurate')
save('data\CRB_vel77_accurate_fs.mat','CRB_vel_accurate')