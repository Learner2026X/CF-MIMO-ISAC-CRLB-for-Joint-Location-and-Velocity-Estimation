...
%             c2 = 2*pi*(1j*N+1)*(t-Delay(n,k,2)-(n-1)*T/N)/T^2; % taking the conjugate
            x1 = (2/T^2)^(1/4)*exp(-pi*(t-Delay(n,k,1)).^2/T^2).*...
                exp(1j*2*pi*(n-1)*delta_f*(t-Delay(n,k,1)));
            x2 = (2/T^2)^(1/4)*exp(-pi*(t-Delay(n,k,2)).^2/T^2).*...
                exp(1j*2*pi*(n-1)*delta_f*(t-Delay(n,k,2)));
            c1 = 2*pi*((t-Delay(n,k,1))/T^2-1j*(n-1)*delta_f);
            c2 = 2*pi*((t-Delay(n,k,2))/T^2+1j*(n-1)*delta_f);% taking the conjugate
...
%             c_a = 4*pi^2/T^4*(1-1j*N)^2*(t-Delay(n,k,1)-(n-1)*T/N).^2-2*pi/T^2*(1-1j*N);         
            ...
            x2 = (2/T^2)^(1/4)*exp(-pi*(t-Delay(n,k,2)).^2/T^2).*...
                exp(1j*2*pi*(n-1)*delta_f*(t-Delay(n,k,2)));
            c1 = 2*pi*((t-Delay(n,k,1))/T^2-1j*(n-1)*delta_f);
            c_a = 4*pi^2*((t-Delay(n,k,1))/T^2-1j*(n-1)*delta_f).^2-2*pi/T^2;
...
%             c_a = 4*pi^2/T^4*(1-1j*N)^2*(t-Delay(n,k,2)-(n-1)*T/N).^2-2*pi/T^2*(1-1j*N);
            ...
            x2 = (2/T^2)^(1/4)*exp(-pi*(t-Delay(n,k,2)).^2/T^2).*...
                exp(1j*2*pi*(n-1)*delta_f*(t-Delay(n,k,2)));
            c2 = 2*pi*((t-Delay(n,k,2))/T^2-1j*(n-1)*delta_f);
            c_a = 4*pi^2*((t-Delay(n,k,2))/T^2-1j*(n-1)*delta_f).^2-2*pi/T^2;
...
CRB_OFDM = CRB_loc;
CRB_OFDM_accurate = CRB_loc_accurate;
CRB_OFDM_single = CRB_loc_single;
save('data\CRB_MT_L_OFDM.mat','CRB_OFDM')
save('data\CRB_MT_L_OFDM_accurate.mat','CRB_OFDM_accurate')
save('data\CRB_MT_L_OFDM_single.mat','CRB_OFDM_single')
toc
%%
x_index = num*space;
figure
